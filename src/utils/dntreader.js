const SimplerReader = require('./simplerreader');
...
'use strict';
    ...
    xhr.onload = function(e) {
      ...
      if (this.status === 200) {
        ...
        if(fileName.toUpperCase().lastIndexOf('.JSON') === fileName.length-5) {
          console.log('trying lzjson');
          var dntFileName = fileName.substr(0,fileName.length-5) + '.lzjson';
  ...
  this.processJsonFile = function(json, fileName) {
    try {
      ...
      throw new Error('dntReader is unable to process a json file: ' + fileName);
    }
